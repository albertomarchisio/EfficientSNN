...
def frame_generator(events_list, number_of_events, accumulation_set, numeric_label):
    ...
    for i in range(n_frames):         
        ...
        # compute the CENTROID for the attention Window:  
        # (I do it only on the ON channel, because the results for the two channels could be slightly different and I would get two different windows for the two channels)
        ...
        # Now I need to define the 64x64 region of the attention window:
        # 1) check that the starting point is at least 0 
        if x_center >=32:
            x_start  = x_center-32   
        else:
            x_start  = 0
        ...
        for i  in range(0,64,2):
            for j in range(0,64,2):
                p_x = int(i/2)  # pixel x
                p_y = int(j/2)  # pixel y
                frame_ON_resize[p_x,p_y]  = (frame_ON[y_start+i, x_start+j] +frame_ON[y_start+i+1, x_start+j] +frame_ON[y_start+i, x_start+j+1] +frame_ON[y_start+i+1, x_start+j+1])/4
                frame_OFF_resize[p_x,p_y] = (frame_OFF[y_start+i, x_start+j] +frame_OFF[y_start+i+1, x_start+j] +frame_OFF[y_start+i, x_start+j+1] +frame_OFF[y_start+i+1, x_start+j+1])/4
...
print('###############################################')
print("saving dataset as: dvs_gesture32x32Window.pickle ...")
with open("dvs_gesture32x32Window.pickle",'wb') as pickle_file:    
