...
def frame_generator(events_list, number_of_events, accumulation_set, numeric_label):
    ...
    for i in range(n_frames):         
        ...
        # I want to resize the image to 32x32, so I need to create 2 matrices (ON and OFF) with dimension 32x32, following the rule:
        #   - if 0 pixel in the 4x4 cluster is active, output pixel will be +(-) 0
        #   - if 1 pixel in the 4x4 cluster is active, output pixel will be +(-) 0.25
        #   - if 2 pixel in the 4x4 cluster is active, output pixel will be +(-) 0.5
        #   - if 3 pixel in the 4x4 cluster is active, output pixel will be +(-) 0.75
        #   - if 4 pixel in the 4x4 cluster is active, output pixel will be +(-) 1
        frame_ON_resize  = np.zeros(shape=(32,32))    
        frame_OFF_resize = np.zeros(shape=(32,32))
        ...
        for i in range(0,128,4):
            for j in range(0,128,4):
                ...
                for k in range(4):
                    for h in range(4):
                        ONpolarity_count  += frame_ON[i+k,j+h]
                        OFFpolarity_count += frame_OFF[i+k,j+h]

                #ONpolarity_count  = frame_ON[i,j]  + frame_ON[i+1,j]  + frame_ON[i,j+1]  + frame_ON[i+1,j+1]
                #OFFpolarity_count = frame_OFF[i,j] + frame_OFF[i+1,j] + frame_OFF[i,j+1] + frame_OFF[i+1,j+1]
                
                #PREDATORpolarity_count = frame_PREDATOR[i,j] + frame_PREDATOR[i+1,j] + frame_PREDATOR[i,j+1] + frame_PREDATOR[i+1,j+1]
                frame_ON_resize[p_x,p_y] =  ONpolarity_count/16 
                frame_OFF_resize[p_x,p_y] = OFFpolarity_count/16

                #frame_ON_resize[p_x,p_y] =  ONpolarity_count/4
                #frame_OFF_resize[p_x,p_y] = OFFpolarity_count/4
                
                #frame_PREDATOR_resize[p_x,p_y] = PREDATORpolarity_count/4

        
        # Normalization:
        max_ON  = np.amax(frame_ON_resize)
        max_OFF = np.amax(frame_OFF_resize)
        #max_PREDATOR = np.amax(frame_PREDATOR_resize)
        frame_ON_resize = np.divide(frame_ON_resize,max_ON)
        frame_OFF_resize = np.divide(frame_OFF_resize,max_OFF)  
        #frame_PREDATOR_resize = np.divide(frame_PREDATOR_resize,max_PREDATOR)  
        
        frame_ON_resize = np.multiply(frame_ON_resize,255)
        frame_OFF_resize = np.multiply(frame_OFF_resize,255)
        #frame_PREDATOR_resize = np.multiply(frame_PREDATOR_resize,255)

        frame = np.zeros(shape=(32,32,2))   # final 2-channel frame that will contain both the accumulation of off and on events

        for x in range(32):
            for y in range(32):
                for i in range(2):
                    if i == 0: 
                        frame[x,y,i] = frame_ON_resize[x,y]     # ON events on channel 1
                    else:
                        frame[x,y,i] = frame_OFF_resize[x,y]    # OFF events on channel 2
...
xdim = 128
ydim = 128
accumulation_set = 1500 #number of events that will produce a single frame
...
print('###############################################')
print("saving dataset as: dvs_gesture32x32_full1500.pickle ...")
with open("dvs_gesture32x32_full1500.pickle",'wb') as pickle_file:    
