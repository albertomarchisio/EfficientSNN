...
def frame_generator_even(events_list, number_of_events, accumulation_set, numeric_label):
    ...
    frame_duration = 300000          #60ms
    channel_duration = 100000
    ...
    for fr_count in range(n_frames):
        ...
        for ch_count in range(n_channels):     
            ...
            #CENTROID computation for the attention Window
            Mx = 0
            My = 0
            mass = 0
            for i in range(128):
            ...
            for i  in range(0,64,2):
                for j in range(0,64,2):
                    p_x = int(i/2)  # pixel x
                    p_y = int(j/2)  # pixel y
                    resized_channel[p_x,p_y]  = (channel[y_start+i, x_start+j] +channel[y_start+i+1, x_start+j] +channel[y_start+i, x_start+j+1] +channel[y_start+i+1, x_start+j+1])/4
    ...
    print("gesture {} completed, {} even frames".format(numeric_label, n_frames))

def frame_generator_odd(events_list, number_of_events, accumulation_set, numeric_label):
    # events_list structure:
    # - list of [x, y, p, ts] for each event
    # x is in the interval [0, 128]
    # y is in the interval [0, 128]
    ...
    frame_duration = 300000          #60ms
    channel_duration = 100000
    start    = events_list[0][3]+150000
    end      = events_list[-1][3]
    lenght   = end-start-150000
    n_frames = math.floor(lenght/frame_duration) # the number of frames is given by the division of the gesture duration (4seconds) by 60ms. That should be 66 frames
    n_channels = 3
    # find the accumulation groups: 
    # for each frame, I have to accumulate a certain number of events, 
    ...
    for fr_count in range(n_frames):
        ...
        #print(datetime.timedelta(seconds=fine-inizio))
    print("gesture {} completed, {} odd frames".format(numeric_label, n_frames))
...
if platform.release() == "5.3.0-28-generic":        # I am on my pc 
...
for aedat in aedat_list:
    ...
    while(end_of_file == False):
        ...
        if collect_enabler == False:
            ...
            # create the frames:
            frame_generator_even(events_list, number_of_events, accumulation_set, class_counter)
            frame_generator_odd(events_list, number_of_events, accumulation_set, class_counter)
...
print('###############################################')
print("saving dataset as: dvs_gesture32x32Window_3chNoPol300msPipe2.pickle ...")
with open("dvs_gesture32x32Window_3chNoPol300msPipe2.pickle",'wb') as pickle_file:    
