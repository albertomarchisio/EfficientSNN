...
def frame_generator(events_list, number_of_events, accumulation_set, numeric_label):
    ...
    # Each of the 6 ouput channels contains 1000 accumulated events. The first channel contains events [0:999], then [1000:1999] and so on..
    # The final tensor will stack the 6 channels and form a single output frame. 

    # DATASET BALANCING POLICY:----------------------
    # the number of frames for each gesture depends on the number of events of that gesture: this makes the gesture unbalanced both on the train and on the test set.
    # In order to balance it, I need to have:
    #    - the same number of frames per each gesture in the train set: the number of frames is the mean between the number of frames of the class that contains more and the number of frames of the class that contains less.
    #         to do so, I remove exceeding frames from classes which have too much and repeate frames from those classes which have less than the desired amount.
    #    - the number of frames per class in the test set has to be equal to the number of  frames of the class which has less. In fact, in the test set I can't repeate the same images, because otherwise I 
    #         would get that the network is evaluated more than one time on one single frame.

    # Analyzing the dataset obtained with the script dataset_generator32x32_6ch.py , I have that:
    #    - The train set has a maximum number of frames of 49 per gessture (obtained by divinding the total number of frames of the gesture with more frames by 97, that is the number of trials)
    #           and a minimum number of frames of 19 per gesture: therefore I want 35 frames per gesture, for each trial
    #    - The test set has a minimum number of frames of 20 per gesture: therefore I want 20 frames per gesture, for each trial


    # n_channels:
    n_channels = 6
    #accumulation_set = 6000
    n_events_per_channel = int(accumulation_set/n_channels)      # 1000

    # number of frames given the defined accumulation_set
    n_frames = math.floor(number_of_events/(accumulation_set))

    if train_set == True:
        final_numer_of_frames = 35
    else:
        final_numer_of_frames = 20


    if n_frames < final_numer_of_frames:
        n_overlaps = math.ceil(final_numer_of_frames/n_frames)
        count_overlaps = 1

    print("gesture {}, have {} frames".format( numeric_label, n_frames))
    for i in range(final_numer_of_frames):

        # select the subset of events of the channel
        
        if n_frames < final_numer_of_frames:
            if i <= n_frames-1:
                if i != n_frames-1:
                    subset_of_events = events_list[(i*accumulation_set) : ((i+1)*accumulation_set-1)]
                else:   # just for the case in which n_frames = final_number_of_frames
                    subset_of_events = events_list[(i*accumulation_set) :]

            elif i > n_frames-1:    # NOW I NEED TO REPLICATE
                restart = (i-n_frames*count_overlaps)
                print("replicating, restart = {}".format(restart))

                if restart != n_frames-1:
                    subset_of_events = events_list[(restart*accumulation_set) : ((restart+1)*accumulation_set-1)]

                else:   # just for the case in which n_frames = final_number_of_frames
                    subset_of_events = events_list[(restart*accumulation_set) :]
                    print("done 1 lap, restarting")
                    count_overlaps += 1


        else:   # NO need to replicate
            if i != n_frames-1:
                subset_of_events = events_list[(i*accumulation_set) : ((i+1)*accumulation_set-1)]
        ...
        for ch in range(n_channels):        
            ...
            channel  = np.full((128,128), 128)  
            
            # select the subset of events fro the channel
            if ch != n_channels-1:
                ch_events = subset_of_events[(ch*n_events_per_channel):((ch+1)*n_events_per_channel-1)]
            else:
                ch_events = subset_of_events[(ch*n_events_per_channel):]

            for event in ch_events:
                if event[2] == 1:     
                    if channel[event[0],event[1]] < 255: # set a maximum accumulation to 128 positive events
                        channel[event[0],event[1]]  += 1   #sum event that happens in the same pixel
                    
                else: 
                    if channel[event[0],event[1]] > 0: # set a maximum accumulation to 128 positive events
                        channel[event[0],event[1]] -= 1   #sum event that happens in the same pixel
            ...
            resized_channel  = np.full((32,32), 128)      
            ...
            resized_channel = np.subtract(resized_channel, np.min(resized_channel))      # subtract minimul value to all elements: I want to clip the values in the interval [0, 255]
            ...
            for x in range(32):
                for y in range(32):
                    frame[x,y,ch] = resized_channel[x,y]
                    if math.isnan(resized_channel[x,y]):
                        print("found nan")
...
print('###############################################')
print("saving dataset as: dvs_gesture32x32_6chBalanced.pickle ...")
with open("dvs_gesture32x32_6chBalanced.pickle",'wb') as pickle_file:    
